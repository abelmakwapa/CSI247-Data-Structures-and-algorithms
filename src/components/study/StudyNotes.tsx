...
export function StudyNotes({ topicId }: { topicId: TopicId }) {
  ...
  return <section className="academic-panel panel-theorem notes-panel" aria-labelledby={`${topicId}-notes-title`}><div className="panel-kicker">Personal notes</div><h2 id={`${topicId}-notes-title`}>Write the explanation you would give in an interview</h2><textarea value={value} onChange={(event) => { setValue(event.target.value); setSaved(false); }} onBlur={() => save(value)} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 's') { event.preventDefault(); save(value); } }} placeholder="Capture a definition, a common trap, or a worked example…" /><p className="notes-status" aria-live="polite">{saved ? 'Saved locally on this device.' : 'Notes save when you leave the field or press Ctrl/Cmd + S.'}</p></section>;
}
