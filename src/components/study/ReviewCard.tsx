...
export function ReviewCard({ item, confidence, explanationVisible, onConfidenceChange, onExplanationToggle, onAnswer }: ReviewCardProps) {
  ...
  return (
    <article className="review-card" aria-labelledby={`review-${item.topic.id}`}>
      ...
      <div className="review-reasons" role="list" aria-label="Reasons this topic is in the queue">
        {item.reasons.map((reason) => <span key={reason} role="listitem">{reason}</span>)}
      </div>
      ...
      <fieldset className="confidence-fieldset">
        ...
        <div className="confidence-scale">
          {([1, 2, 3, 4, 5] as const).map((rating) => <button key={rating} type="button" className={confidence === rating ? 'selected' : ''} aria-label={`Confidence ${rating} of 5${rating === 1 ? ', low' : rating === 5 ? ', high' : ''}`} aria-pressed={confidence === rating} onClick={() => onConfidenceChange(rating)}><span aria-hidden="true">{rating}</span><small aria-hidden="true">{rating === 1 ? 'Low' : rating === 5 ? 'High' : ''}</small></button>)}
        </div>
      </fieldset>
