...
import type { DiagramKind, TopicId } from '@/lib/topics';
...
const STEP_COPY = [
  'Orient yourself: find the invariant or the part of the structure that controls the next move.',
  'Follow one operation: the highlighted item is where the algorithm touches or changes state.',
  'Watch the boundary: the visual shows what is discarded, shifted, linked, or remembered.',
  'Name the cost: connect the number of touched items to the complexity bound.',
];
...
function DiagramContent({ kind, step }: { kind: DiagramKind; step: number }) {
  switch (kind) {
    case 'array':
      return <><text x="20" y="24" className="atlas-muted">CONTIGUOUS MEMORY · INDEXED SLOTS</text>{[10, 24, 31, 48, 57, 63].map((value, index) => <g key={value}><Box x={30 + index * 90} y={72} label={String(value)} active={step === 1 && index === 2} muted={step === 2 && index < 2} /><text x={66 + index * 90} y="137" textAnchor="middle" className="atlas-muted">{index}</text></g>)}<Line x1={255} y1={158} x2={255} y2={202} dashed={step === 1} /><text x="275" y="198" className="atlas-accent atlas-label">base + index × slot</text></>;
    case 'chain':
      return <><text x="20" y="24" className="atlas-muted">NODES CAN LIVE ANYWHERE · POINTERS KEEP ORDER</text>{['A', 'B', 'C', 'D'].map((label, index) => <g key={label}><Box x={34 + index * 138} y={80} label={label} active={step === 2 && index === 1} /><Line x1={106 + index * 138} y1={102} x2={164 + index * 138} y2={102} dashed={step === 1 || step === 2} /></g>)}<text x="34" y="160" className="atlas-muted">head</text><text x="440" y="160" className="atlas-muted">tail</text></>;
    case 'stack':
      return <><text x="20" y="24" className="atlas-muted">TOP ONLY · LAST IN, FIRST OUT</text>{['10', '8', '3'].map((label, index) => <Box key={label} x={218} y={58 + index * 50} width={150} label={label} active={(step === 1 && index === 0) || (step === 2 && index === 1)} muted={step === 2 && index === 0} />)}<Line x1={390} y1={80} x2={470} y2={80} dashed={step === 1 || step === 2} /><text x="402" y="70" className="atlas-accent atlas-label">pop</text><text x="207" y="222" className="atlas-muted">push and pop share one boundary</text></>;
    case 'queue':
      return <><text x="20" y="24" className="atlas-muted">FIFO · FRONT SERVES · REAR ARRIVES</text>{['A', 'B', 'C', 'D'].map((label, index) => <Box key={label} x={72 + index * 110} y={82} label={label} active={(step === 1 && index === 3) || (step === 2 && index === 0)} muted={step === 2 && index === 0} />)}<Line x1={30} y1={104} x2={64} y2={104} dashed={step === 2} /><Line x1={520} y1={104} x2={584} y2={104} dashed={step === 1} /><text x="24" y="154" className="atlas-muted">dequeue</text><text x="520" y="154" className="atlas-muted">enqueue</text></>;
    case 'deque':
      return <><text x="20" y="24" className="atlas-muted">TWO LIVE ENDS · BOTH BOUNDARIES ARE ACTIONABLE</text>{['A', 'B', 'C', 'D'].map((label, index) => <Box key={label} x={100 + index * 105} y={86} label={label} active={(step === 1 && index === 0) || (step === 2 && index === 3)} />)}<Line x1={78} y1={108} x2={40} y2={108} dashed={step === 1} /><Line x1={522} y1={108} x2={580} y2={108} dashed={step === 2} /><text x="28" y="158" className="atlas-accent atlas-label">front</text><text x="518" y="158" className="atlas-accent atlas-label">back</text></>;
    case 'hash-map':
    ...
    case 'set':
      return <><text x="20" y="24" className="atlas-muted">MEMBERSHIP · UNIQUE KEYS ONLY</text>{['Ada', 'Linus', 'Grace', 'Ken'].map((label, index) => <Box key={label} x={34 + index * 138} y={76} width={116} label={label} active={step === 1 && index === 1} />)}<rect x="160" y="162" width="148" height="30" rx="5" className="atlas-node-focus" /><text x="234" y="182" textAnchor="middle" className="atlas-accent atlas-label">has(&apos;Linus&apos;) ✓</text><Line x1={218} y1={134} x2={234} y2={158} dashed={step === 2} /></>;
    case 'tree':
    case 'bst':
      return <><text x="20" y="24" className="atlas-muted">{kind === 'bst' ? 'ORDER DISCARDS A SUBTREE' : 'PARENT → CHILD → LEAF'}</text><Line x1={310} y1={70} x2={310} y2={108} /><Line x1={310} y1={108} x2={170} y2={154} /><Line x1={310} y1={108} x2={450} y2={154} /><Line x1={170} y1={154} x2={100} y2={198} /><Line x1={170} y1={154} x2={240} y2={198} /><Line x1={450} y1={154} x2={380} y2={198} /><Line x1={450} y1={154} x2={520} y2={198} /><circle cx="310" cy="54" r="24" className={focus(step, 1)} /><text x="310" y="58" textAnchor="middle" className="atlas-label">{kind === 'bst' ? '50' : 'root'}</text>{[['docs', 170, 140], ['media', 450, 140], ['readme', 100, 185], ['notes', 240, 185], ['photos', 380, 185], ['video', 520, 185]].map(([label, x, y], index) => <g key={String(label)} opacity={step === 2 && index > 1 ? .35 : 1}><circle cx={Number(x)} cy={Number(y)} r="22" className={step === 2 && index === 0 ? 'atlas-node-focus' : 'atlas-node'} /><text x={Number(x)} y={Number(y) + 4} textAnchor="middle" className="atlas-label">{label}</text></g>)}</>;
    case 'heap':
      return <><text x="20" y="24" className="atlas-muted">COMPLETE SHAPE · PRIORITY AT ROOT</text><Line x1={310} y1={70} x2={310} y2={110} /><Line x1={310} y1={110} x2={180} y2={155} /><Line x1={310} y1={110} x2={440} y2={155} /><Line x1={180} y1={155} x2={115} y2={198} /><Line x1={180} y1={155} x2={245} y2={198} /><circle cx="310" cy="54" r="25" className={step === 1 ? 'atlas-node-focus' : 'atlas-node'} /><text x="310" y="58" textAnchor="middle" className="atlas-accent atlas-label">100</text>{[['80', 180, 140], ['65', 440, 140], ['30', 115, 185], ['20', 245, 185]].map(([label, x, y], index) => <g key={String(label)}><circle cx={Number(x)} cy={Number(y)} r="21" className={step === 2 && index === 0 ? 'atlas-node-focus' : 'atlas-node'} /><text x={Number(x)} y={Number(y) + 4} textAnchor="middle" className="atlas-label">{label}</text></g>)}</>;
    case 'graph':
      return <><text x="20" y="24" className="atlas-muted">RELATIONSHIPS · CYCLES ARE NORMAL</text><Line x1={150} y1={92} x2={310} y2={58} /><Line x1={310} y1={58} x2={478} y2={92} /><Line x1={150} y1={92} x2={198} y2={184} /><Line x1={198} y1={184} x2={388} y2={184} /><Line x1={388} y1={184} x2={478} y2={92} /><Line x1={310} y1={58} x2={388} y2={184} />{[['A', 150, 92], ['B', 310, 58], ['C', 478, 92], ['D', 198, 184], ['E', 388, 184]].map(([label, x, y], index) => <g key={String(label)}><circle cx={Number(x)} cy={Number(y)} r="26" className={step === 1 && index === 0 ? 'atlas-node-focus' : 'atlas-node'} /><text x={Number(x)} y={Number(y) + 4} textAnchor="middle" className="atlas-label">{label}</text></g>)}<Line x1={118} y1={92} x2={82} y2={92} dashed={step === 1 || step === 2} /></>;
    case 'trie':
      return <><text x="20" y="24" className="atlas-muted">SHARED PREFIXES · ONE EDGE PER CHARACTER</text><Line x1={310} y1={66} x2={210} y2={108} /><Line x1={310} y1={66} x2={410} y2={108} /><Line x1={210} y1={108} x2={150} y2={160} /><Line x1={210} y1={108} x2={270} y2={160} /><Line x1={410} y1={108} x2={350} y2={160} /><Line x1={410} y1={108} x2={470} y2={160} />{[['∅', 310, 52], ['a', 210, 98], ['b', 410, 98], ['p', 150, 150], ['d', 270, 150], ['t', 350, 150], ['y', 470, 150]].map(([label, x, y], index) => <g key={String(label)}><circle cx={Number(x)} cy={Number(y)} r="22" className={step === 2 && index === 3 ? 'atlas-node-focus' : 'atlas-node'} /><text x={Number(x)} y={Number(y) + 4} textAnchor="middle" className="atlas-label">{label}</text></g>)}<text x="116" y="208" className="atlas-muted">app</text><text x="238" y="208" className="atlas-muted">add</text></>;
    case 'dsu':
    ...
    case 'bloom':
      return <><text x="20" y="24" className="atlas-muted">HASHES → BITS · NO FALSE NEGATIVES</text><Box x={36} y={86} width={134} label="/checkout" active={step === 1} /><Line x1={174} y1={108} x2={245} y2={108} dashed={step === 1 || step === 2} />{Array.from({ length: 12 }, (_, index) => <rect key={index} x={258 + index * 26} y={86} width={19} height={48} rx={3} className={[1, 4, 8, 10].includes(index) ? 'atlas-node-focus' : 'atlas-node'} />)}<text x="258" y="164" className="atlas-muted">0 1 0 0 1 0 0 0 1 0 1 0</text><text x="36" y="206" className="atlas-accent atlas-label">zero bit → definitely absent</text></>;
    case 'lru':
    ...
    case 'sorting':
      return <><text x="20" y="24" className="atlas-muted">COMPARE → PLACE → SORTED PREFIX GROWS</text>{[7, 3, 6, 2, 5].map((value, index) => <Box key={value} x={42 + index * 102} y={150 - value * 9} width={68} height={value * 9} label={String(value)} active={step === 1 && (index === 1 || index === 2)} muted={step === 2 && index > 1} />)}<Line x1={140} y1={96} x2={242} y2={96} dashed={step === 1 || step === 2} /><text x="40" y="214" className="atlas-muted">ordered prefix</text></>;
    case 'binary-search':
      return <><text x="20" y="24" className="atlas-muted">TARGET 44 · KEEP ONLY THE POSSIBLE INTERVAL</text>{[4, 9, 15, 22, 31, 44, 58].map((value, index) => <Box key={value} x={34 + index * 78} y={88} width={60} label={String(value)} active={(step === 1 && index === 3) || (step === 2 && index === 5)} muted={step === 2 && index < 3} />)}<Line x1={306} y1={140} x2={514} y2={182} dashed={step === 2} /><text x="310" y="202" className="atlas-accent atlas-label">44 is higher than midpoint → right half</text></>;
    case 'recursion':
...
export function TopicDiagram({ topicId }: { topicId: TopicId }) {
  ...
  function exportSvg() {
    ...
    link.href = url;
    link.download = `${topicId}-explainer.svg`;
  ...
  return (
    <figure className="diagram-panel academic-panel panel-definition" aria-labelledby={titleId}>
      <div className="diagram-head"><div><div className="panel-kicker">Visual explainer</div><h2 id={titleId}>See the invariant change</h2></div><button type="button" className="text-button" onClick={exportSvg}>Export SVG</button></div>
      <div className="diagram-canvas"><svg ref={svgRef} viewBox="0 0 620 250" role="img" aria-labelledby={titleId}><title>Visual explainer</title><defs><marker id="atlas-arrow" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0 0 L0 6 L7 3Z" fill="#c8973a" /></marker></defs><DiagramContent kind={topic.diagram} step={step} /><g transform="translate(20 220)"><rect width="24" height="20" rx="4" className="atlas-step-bg" /><text x="12" y="14" textAnchor="middle" className="atlas-step">{step + 1}</text><text x="34" y="14" className="atlas-callout">Step {step + 1} of 4</text></g></svg></div>
      <figcaption><div className="diagram-step-header"><strong>{['Orient', 'Operate', 'Trace', 'Name the cost'][step]}</strong><span>{step + 1} / 4</span></div><p>{STEP_COPY[step]}</p><div className="diagram-controls"><button type="button" onClick={() => setStep((value) => Math.max(0, value - 1))} disabled={step === 0}>← Previous</button>{STEP_COPY.map((_, index) => <button key={index} type="button" className={`step-dot${step === index ? ' active' : ''}`} onClick={() => setStep(index)} aria-label={`Show diagram step ${index + 1}`} aria-pressed={step === index} />)}<button type="button" onClick={() => setStep((value) => Math.min(3, value + 1))} disabled={step === 3}>Next →</button></div></figcaption>
    </figure>
  );
}
